...
| ВК (стена сообщества) | `VK_TOKEN`, `VK_GROUP_ID` | **только текст**: ключ сообщества не умеет загружать фото и видео (проверено `python3 tools/vk_check.py`), поэтому вместо медиа в пост добавляется ссылка на оригинал в Telegram; нужное право ключа — «стена»; для настоящих фото нужен пользовательский токен, который ВК выдаёт по запросу в поддержку |
